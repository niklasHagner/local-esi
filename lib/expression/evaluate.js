...
module.exports = function evaluate(ast, context) {
  const funcs = {
    ...
    DictionaryPropertyAccessor(node) {
      return node.cargo;
    }
  };
  ...
  function getFunc(name) {
    if (!funcs[name]) {
      const isAstMemberExpressionIdentifier = ast && ast.type === "MemberExpression" && ast.object && ast.object.type === "Identifier";
      const dictionaryVariableName = ast.object.name;
      if (isAstMemberExpressionIdentifier && context && context.assigns && typeof context.assigns[dictionaryVariableName][name] !== undefined) {
        return funcs.DictionaryPropertyAccessor;
      }
